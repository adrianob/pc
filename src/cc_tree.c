...
#define ERRO(MENSAGEM)                                                         \
    {                                                                          \
        fprintf(stderr, "[cc_tree, %s] %s.\n", __FUNCTION__, MENSAGEM);        \
        abort();                                                               \
    }
...
comp_tree_t *tree_make_binary_node(void *value, comp_tree_t *node1,
                                   comp_tree_t *node2) {
    comp_tree_t *newnode = tree_make_node(value);
...
comp_tree_t *tree_make_ternary_node(void *value, comp_tree_t *node1,
                                    comp_tree_t *node2, comp_tree_t *node3) {
    comp_tree_t *newnode = tree_make_node(value);
    tree_insert_node(newnode, node1);
    tree_insert_node(newnode, node2);
...
void tree_debug_print(comp_tree_t *tree) { tree_debug_print_s(tree, 0); }
