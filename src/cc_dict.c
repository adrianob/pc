...
void *dict_put(comp_dict_t *dict, char *key, void *value) {
    ...
    if (dict->data[hash] == NULL) { // caso 1: entrada não existe na tabela, é
...
void *dict_remove(comp_dict_t *dict, char *key) {
    ...
    if (dict->data[hash]) {
        if (strcmp(dict->data[hash]->key, key) ==
            0) { // chave é primeiro elemento
            item = dict->data[hash];
            data = item->value;
            ...
            if (dict->data[hash] ==
                NULL) // chave foi ultimo elemento da entrada
                --dict->occupation;
