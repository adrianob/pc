...
static inline void __gv_test_valid_fp(const char *function_name) {
    if (!fp) {
        fprintf(stderr,
                "%s foi chamada, mas gv_init não foi chamada antes, abort()\n",
                function_name);
        abort();
    }
}

static inline void __gv_test_valid_ast_pointer(const char *function_name,
                                               const char *pointer) {
    if (!pointer) {
...
void gv_init(const char *filename) {
    ...
    if (filename) {
        ...
        if (!fp) {
            fprintf(stderr,
                    "%s:%d não conseguiu abrir o arquivo %s para escrita\n",
